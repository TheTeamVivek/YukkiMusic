...
async def joke(_, message):
    ...
    await db.tap_count.insert_one(
        {"joke_id": joke_id, "tap_count": 0, "last_tapped_user": None}
    )

    refresh_button = InlineKeyboardButton(
        "ʀᴇғʀᴇsʜ", callback_data=f"refresh_joke_{joke_id}"
    )
...
async def refresh_joke(_, query):
    ...
    await db.tap_count.update_one(
        {"joke_id": joke_id}, {"$set": {"last_tapped_user": last_tapped_user}}
    )
    ...
    await query.message.edit_text(
        new_joke_text
        + f"\n\nᴛᴏᴛᴀʟ ᴛᴀᴘ: {tap_count}\nʟᴀsᴛ ᴛᴀᴘᴘᴇᴅ ʙʏ: {last_tapped_user}",
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("ʀᴇғʀᴇsʜ", callback_data=f"refresh_joke_{joke_id}")]]
        ),
