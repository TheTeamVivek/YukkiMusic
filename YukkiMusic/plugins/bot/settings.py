...
async def gen_buttons_aud(_, aud):
    if aud == "Very High":
        buttons = audio_quality_markup(_, very_high=True)
    elif aud == "High":
...
async def gen_buttons_vid(_, aud):
    if aud == "Very High":
        buttons = video_quality_markup(_, very_high=True)
    elif aud == "High":
...
@app.on_callback_query(
    filters.regex(pattern=r"^(LQA|MQA|HQA|VHQA|LQV|MQV|HQV|VHQV)$")
    & ~BANNED_USERS
)
@ActualAdminCB
async def aud_vid_cb(client, CallbackQuery, _):
    ...
    if command == "VHQA":
        await save_audio_bitrate(
            CallbackQuery.message.chat.id, "Very High"
        )
        buttons = audio_quality_markup(_, very_high=True)
    if command == "LQV":
