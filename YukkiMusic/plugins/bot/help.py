...
async def paginate_modules(page_n, chat_id: int, close: bool = False):
    ...
    pairs = [all_buttons[i: i + NUM_COLUMNS] for i in range(0, len(all_buttons), NUM_COLUMNS)]
...
async def help_button(client, query):
    ...
    if mod_match:
        ...
        await query.message.edit(
            text=top_text,
            reply_markup=await paginate_modules(curr_page, query.message.chat.id, close=close),
            disable_web_page_preview=True,
        )
    elif next_match:
        ...
        await query.message.edit(
            text=top_text,
            reply_markup=await paginate_modules(next_page, query.message.chat.id, close=close),
            disable_web_page_preview=True,
        )
    elif helper_match:
