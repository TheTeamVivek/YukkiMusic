...
async def mute_admin(cli, message: Message, _, chat_id):
    ...
    if await is_muted(chat_id):
        return await message.reply_text(_["admin_5"], link_preview=False)
    await mute_on(chat_id)
    await Yukki.mute_stream(chat_id)
    await message.reply_text(
        _["admin_6"].format(message.from_user.mention), link_preview=False
    )
...
async def unmute_admin(Client, message: Message, _, chat_id):
    ...
    if not await is_muted(chat_id):
        return await message.reply_text(_["admin_7"], link_preview=False)
    await mute_off(chat_id)
    await Yukki.unmute_stream(chat_id)
    await message.reply_text(
        _["admin_8"].format(message.from_user.mention), link_preview=False
    )
