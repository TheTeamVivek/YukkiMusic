...
def CPlayWrapper(command):
    async def wrapper(client, message):
        ...
        if not await is_active_chat(chat_id):
            ...
            try:
                try:
                    await userbot.send_message(me.username, "/start")
                    check_master = await client.get_chat_member(chat_id, app.username)
                ...
                if chat_id in links:
                    ...
                    if message.chat.username:
                        ...
                        try:
                            ...
                            return await message.reply_text(
                                _["call_3"].format(me.mention, type(e).__name__)
                            )
                ...
                try:
                    ...
                    await asyncio.sleep(3)
                    await myu.edit(_["call_5"].format(me.mention))
                except UserAlreadyParticipant:
                    pass
                except Exception as e:
                    return await message.reply_text(
